...
TMOD&=0x0F;TMOD|=0x20;	// Set Timer1 to Mode 2 (8-bit auto reload) for Baud Rate Generation
...
void uart_put_float(float float_data, int Decimal_len){
    if(float_data<0){
        float_data*=-1;
        uart_put_char('-');
    }
    uart_put_integer((int) float_data);
    uart_put_char('.');
    while(Decimal_len-- > 0){
        float_data*=10;
